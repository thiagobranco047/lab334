...
The application consumes `@lab334/contracts@0.1.4` from the checksummed generated archive in `vendor/contracts/`. This archive is not editable source. Update it only by running the canonical generator in `lab334-agents`, copying `.tgz` and `.sha256` byte-for-byte, running `npm install`, and then `npm run contracts:verify`.
