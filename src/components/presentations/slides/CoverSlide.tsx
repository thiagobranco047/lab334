...
export default function CoverSlide({ slide, presentation }: SlideProps) {
  return (
    ...
    >
      <motion.p variants={fadeUpItem} className="font-body text-base font-light text-proposal-muted sm:text-lg">
        Cliente: <span className="text-proposal-fg">{presentation.client}</span>
      </motion.p>

      <motion.div variants={fadeUpItem} className="mt-10 border-t border-proposal-line pt-10">
        ...
        <h1 className="mt-4 max-w-slide-title lg:max-w-slide-title-lg font-display text-[clamp(3rem,8vw,5.5rem)] font-extralight leading-[0.95] tracking-lab text-proposal-fg">
          {slide.title}
        </h1>
      </motion.div>
